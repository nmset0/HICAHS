...
The purpose of this project is to evaluate the correlation between aspects of migrant agricultural worker population and environmental risk within the states of interest to the High Plains Intermountain Center for Agricultural Health and Safety [(HICAHS)](https://vetmedbiosci.colostate.edu/hicahs/). HICAHS is involved with Colorado, Wyoming, Utah, Montana, North Dakota, and South Dakota.

The Federal Emergency Management Agency (FEMA) defines exposure to some natural disaster as "...the representative value of buildings (in dollars), population (in both people and population equivalence dollars), or agriculture (in dollars) potentially exposed to a natural hazard occurrence." In other words, locations with higher building value, agricultural value, and/or population density would have higher exposure values associated with them. For more information, visit [FEMA's website](https://hazards.fema.gov/nri/exposure).

The H-2A visa allows agricultural firms, in lieu of a labor shortage, to bring in and employ foreign workers on a temporary or seasonal basis. For more information, visit the [United States Department of Labor](https://www.dol.gov/agencies/eta/foreign-labor/programs/h-2a#:~:text=There%20are%20not%20sufficient%0able%2C%20willing%2C%20and%20qualified%20United%20States,employment%20to%20eligible%20U.S.%20workers.).

A migrant health center (MHC) is a specialized healthcare facility that provides services to migrant agricultural workers and their families. MHCs were established in 1962 after President John F. Kennedy signed the [Migrant Health Act](https://www.ncfh.org/uploads/3/8/6/8/38685499/migrant_health_history_and_legislation_final.pdf), initializing the Migrant Health Center program. MHCs receive funding under the Public Health Service Act.

Statistical significance is the probability that a test's null hypothesis to be true. The p-value signifies the probability of finding test results as, or more, extreme than observed data, assuming the null hypothesis is true. The p-value must be equal to or below a certain threshold (Usually five percent, or p $\le$ 0.05) of uncertainty for the results to be statistically significant. If the p-value is greater than the confidence level, the results are considered not statistically significant.

## Research Goals

|        1) Investigate the relationship between health risk concentrations and healthcare market supply,
|        particularly the supply of migrant health centers
| 
|        2) Investigate the relationship between health risk factors and worker densities in the HICAHS area
| 
|        3) Investigate the relationship between health risk concentrations and economic output of agriculture.

## Exploration

The figures below show the distribution of MHCs and H-2A population density in the HICAHS area In total. There are 56 health centers between all six states, with Colorado having the most. South Dakota and Wyoming have none at all. In total, 12,893 H-2A agricultural workers reside within the HICAHS area.
...
GGDAT1$state <- factor(GGDAT1$state, levels = c("Colorado", "Montana", "Utah", "North Dakota", "South Dakota", "Wyoming"))
...
GGPLOT2 <- ggplot(data = GGDAT2, aes(y = state, x = as.numeric(total))) + 
  ...
  scale_x_continuous(breaks = seq(from = 0, to = 4000, by = 1000)) + 
...
color_palette <- colorNumeric(palette = c("yellow", "orange1", "red"), domain = JOIN_CUT_sf$H2A_workers)
map <- leaflet(MHC) |>
  addTiles() |>
...
The correlation between the population of workers and MHCs is dangerously low (and statistically significant), signaling a vastly outclassed healthcare supply for migrant workers and their families.
...
kable(STATE_MHC_CORR, align = c("r", "c", "c", "c", "r"))
```

## Correlation between H-2A population and wildfire, drought, and heat wave exposure, and temperature

Most of the correlation coefficients found between H-2A worker density and exposure risk is not surprising. There are some that are surprisingly weak, but these are also not statistically significant.
...
These correlations are not very strong. Healthcare companies take environmental exposure risk into account less so than factors such as population density. The figures below contain the correlation coefficients between healthcare and exposure. Note that 47.9 percent of these coefficients are not statistically significant.
...
These correlations are also unsurprising for the most part. A firm's value is expected to be associated with their income/number of sales, so any location with high value will also have a high exposure score. The figures below contain the correlation coefficients between output and exposure.

It would be expected variables pertaining to high-value transactions be more correlated with exposure-- especially agricultural exposure-- however, some of the transaction variables have very little correlation, with some of the environmental risks. For example, the dollar value of agriculture exposed to heat waves and farm sales between \$10000 and \$24999 have a correlation coefficient of 0.70. These variables in particular would be expected to have a strong positive correlation. That being said, the correlation coefficient was not statistically significant.
...
2)  [Wyoming Department of Health](https://health.wyo.gov/aging/hls/healthcare-facility-directory/)

3)  [Utah Geospatial Resource Center](https://gis.utah.gov/data/health/health-care-facilities/)

4)  [South Dakota Department of Health](https://www.sdhls.org/verify/)

5)  [North Dakota Health and Human Services](https://www.hhs.nd.gov/)

6)  [Montana Department of Health and Human Services](https://www.dphhs.mt.gov/qad/Licensure/)

7)  [Montana Primary Care Association](https://www.mtpca.org/wp-content/uploads/CHC_locations_MT_2022.pdf)

8)  [Colorado Department of Public Health and Environment](https://cdphe.colorado.gov/health-facilities/find-and-compare-facilities)

9)  Migrant Health Centers: [National Center for Farmworker Health, Inc. (2023)](https://www.ncfh.org/migrant-health-centers.html)

10)  Agricultural Census: [U.S. Department of Agriculture (2007-2023)](https://www.nass.usda.gov/Quick_Stats/CDQT/chapter/2/table/42/state/CO/year/2017/)

11)  Maximum Temperature by County: [National Oceanic and Atmospheric Association (NOAA), 2023](https://www.ncei.noaa.gov/access/monitoring/climate-at-a-glance/county/mapping/5/tmax/202208/1/value)

12)  [National Risk Index](https://hicahs-centroid-collaboration-csurams.hub.arcgis.com/apps/1ff51926aada4a85a74ee89ba1a2539b/explore): Federal Emergency Management Agency (ArcGIS Living Atlas)
