...
ag_output_wide <- ag_output %>%
  pivot_wider(id_cols = c(state, county), names_from = data.item, values_from = value)
...
disaster <- disaster |> select(-National.Risk.Index.ID, -GlobalID, -National.Risk.Index.Version)
